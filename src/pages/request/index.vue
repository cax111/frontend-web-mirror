  ...
  <div class="relative q-ma-md">
    <div class="bg-grey-2 fixed row q-gutter-sm q-pb-md search-box">
      <q-breadcrumbs class="text-grey-7 col-12">
        ...
        <q-breadcrumbs-el label="Request" icon="fas fa-book" />
      </q-breadcrumbs>
      <q-input
    ...
    <div class="text-primary" style="padding-top: 80px">
      ...
      <q-list v-else padding>
        <q-item
          v-for="(item, i) in request.filter((val) =>
            val.lesson.toLowerCase().includes(search.toLowerCase())
          )"
          :key="i"
          clickable
          v-ripple
          ...
          style="border-radius: 12px"
          @click="$router.push(`/questions/request/${item.id}`)"
        >
          <q-item-section avatar>
            <q-icon name="fas fa-book" size="md" />
          </q-item-section>
          <q-item-section class="col">{{ item.lesson }}</q-item-section>
          <q-item-section avatar>
